...
class CrawlerManager {
  ...
  scheduleCrawl(websiteRecord) {
    if (this.activeExecutions.has(websiteRecord.id)) {
      ...
      if (worker) {
        console.log(
          `Cancelling previous execution for record ${websiteRecord.id}.`
        );
        worker.terminate();
      }
      if (timeout) {
  ...
  cancelExecution(websiteRecordId) {
    ...
    if (activeExecution) {
      ...
      if (worker) {
        ...
        console.log(
          `Worker for record ${websiteRecordId} has been terminated.`
        );
      }
      if (timeout) {
    ...
    worker.on("message", async (message) => {
      if (message.crawledData) {
        ...
        if (websiteRecord.isActive) {
          ...
          this.activeExecutions.set(websiteRecord.id, {
            worker: null,
            timeout,
          });
        } else {
