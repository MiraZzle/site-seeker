<script lang="ts">
  ...
  function removeRecordFromUI(id: number) {
    ...
    filteredWebsiteRecords = filteredWebsiteRecords.filter((record) => record.id !== id);
    displayedRecords = displayedRecords.filter((record) => record.id !== id);
  }
  ...
  function loadRecords(newPage: number) {
    ...
    filteredWebsiteRecords = websiteRecords.filter((record) => {
      const matchesURL = !filterURL || record.url.toLowerCase().includes(filterURL.toLowerCase());
      const matchesLabel =
        !filterLabel || record.label.toLowerCase().includes(filterLabel.toLowerCase());
      const matchesTags =
        filterTags.length === 0 ||
        filterTags.every((tag) =>
          record.tags.some((recordTag) => recordTag.toLowerCase() === tag.toLowerCase())
...
<div class="records-cta">
  ...
  <Button type="primary" actionType="action" action={() => showCreateRecordModal()}>
